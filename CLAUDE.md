...
The authoritative coding rules live in `.agents/rules/`. Read the relevant rule
...
`main` mirrors the upstream NVIDIA repo; `dev-clean` carries the research
work. Before porting anything between them, read `PORTING.md` — the living
ledger of intentional divergences (custom code, frozen environment pins,
hand-merge files) and the post-port verification checklist.
...
- **Ensemble-pipeline figures** — before writing or modifying an `aggregate_*.py` /
  `compute_*.py` in `scripts/ensemble_analysis/`, read
  `scripts/_shared/README.md` for the shared figure helpers (local-time axes,
  MathText units, paper fontsizes, axis-limit lookups) and case-YAML schema.
