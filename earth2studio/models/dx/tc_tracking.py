...
VARIABLES_TCMSL = ["u10m", "v10m", "msl"]

# Base fields required by the TempestExtremes tracker (no warm-core test).
VARIABLES_TEMPEST_BASE = ["u10m", "v10m", "msl"]

# Extra fields needed for the canonical Zarzycki & Ullrich (2017) warm-core
# closed-contour test on the 300-500 hPa geopotential thickness.  Archived
# ensemble runs that don't persist z300 can't use these.
VARIABLES_TEMPEST_WARM_CORE_EXTRAS = ["z300", "z500"]


class TCTrackerMinMSL(torch.nn.Module, _TCTrackerBase):
    """Finds tropical cyclone centers by locating local minima in the mean sea
    level pressure (MSL) field.

    This is a simpler alternative to vorticity-based trackers such as
    :class:`TCTrackerWuDuan`.  It detects local MSL minima below a given
    threshold and extracts the 10-m wind speed at that location.

    Parameters
    ----------
    msl_threshold : float, optional
        MSL values (Pa) must be below this to be considered a TC candidate.
        Default is 100500 Pa (1005 hPa).
    min_distance : int, optional
        Minimum pixel distance between detected minima, by default 20.
    path_search_distance : int, optional
        Max haversine search distance in km when connecting centers into
        paths, by default 300.
    path_search_window_size : int, optional
        Historical window size for path pairing, by default 2.

    Examples
    --------
    >>> model = TCTrackerMinMSL()
    >>> for time in times:
    ...     output, output_coords = model(input, input_coords)
    >>> model.path_buffer.shape  # [batch, path_id, steps, 4]
    """

    def __init__(
        self,
        msl_threshold: float = 100500.0,
        min_distance: int = 20,
        path_search_distance: int = 300,
        path_search_window_size: int = 2,
    ) -> None:
        super().__init__()
        self.register_buffer("path_buffer", torch.empty(0))
        self.msl_threshold = msl_threshold
        self.min_distance = min_distance
        self.path_search_distance = path_search_distance
        self.path_search_window_size = path_search_window_size

    def reset_path_buffer(self) -> None:
        """Resets the internal path buffer."""
        self.path_buffer = torch.empty(0)
    ...
    def input_coords(self) -> CoordSystem:
        ...
        return OrderedDict(
            {
                "batch": np.empty(0),
                "variable": np.array(VARIABLES_TCMSL),
                "lat": np.linspace(90, -90, 721, endpoint=True),
                "lon": np.linspace(0, 360, 1440, endpoint=False),
            }
        )
    ...
    def _find_centers(
        ...
        """Find TC centers as local MSL minima.
        ...
        msl : torch.Tensor
            Mean sea level pressure of shape [lat, lon].
        w10m : torch.Tensor
            10-m wind speed of shape [lat, lon].

        Returns
        -------
        torch.Tensor
            TC centers of shape [N, 4] (lat, lon, msl, w10m).
        """
        # Negate MSL so local maxima correspond to pressure minima
        neg_msl = -msl
        # threshold_abs for negated field: we want msl < msl_threshold,
        # i.e. -msl > -msl_threshold
        local_max = self.get_local_max(
            neg_msl,
            threshold_abs=-self.msl_threshold,
            min_distance=self.min_distance,
            exclude_border=True,
        )

        if local_max.numel() == 0:
            return torch.full((1, 4), self.PATH_FILL_VALUE, device=msl.device)

        centers = []
        for k in range(local_max.shape[0]):
            lat_idx = int(local_max[k, 0])
            lon_idx = int(local_max[k, 1])
            lat_c = lat[lat_idx]
            lon_c = lon[lon_idx]

            # Skip near-equatorial detections
            if abs(float(lat_c)) < 5:
                continue

            msl_val = msl[lat_idx, lon_idx]
            w10m_val = w10m[lat_idx, lon_idx]
            centers.append(
                torch.as_tensor(
                    [float(lat_c), float(lon_c), float(msl_val), float(w10m_val)],
                    device=msl.device,
                )
            )
    ...
    def __call__(
        ...
        def get_variable(x: torch.Tensor, var: str) -> torch.Tensor:
            index = VARIABLES_TCMSL.index(var)
            return x[index]
        ...
        for i in range(x.shape[0]):
            ...
            centers = self._find_centers(lat, lon, msl, w10m)
            outs.append(centers)
