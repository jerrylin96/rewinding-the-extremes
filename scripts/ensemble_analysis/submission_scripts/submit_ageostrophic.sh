...
# ---------------------------------------------------------------------
# BU SCC SGE Directives — Ageostrophic fraction line plot, full ensemble
#
# Case-agnostic wrapper: all paths/args are forwarded from
# dispatch_ageostrophic.py so the same script handles Sandy, PNW
# heatwave, etc.  Run directly only if you want to override the
# defaults that the dispatcher would otherwise compute.
# ---------------------------------------------------------------------
#$ -P eb-general              # Project Name
#$ -N Ageo                    # Job Name
#$ -cwd                       # Run from the submission directory
#$ -j y                       # Merge Output/Error (file written to ageostrophic_logs/, see below)
# h_rt is set via qsub -l flags from dispatch_ageostrophic.py
#$ -pe omp 8                  # 8 CPU cores (numpy/torch threads + zarr decode)
#$ -l mem_per_core=16G        # 128 GB total — comfortable for batch_size=32
...
PYTHON_SCRIPT="${SCRIPT_DIR}/compute_ageostrophic.py"
...
# ---------------------------------------------------------------------
# 2. Forward all arguments to compute_ageostrophic.py
#
# Expected arguments from dispatch_ageostrophic.py:
#   --ensemble-zarr <path>
#   --era5-zarr <path>
#   --output-dir <path>
...
#   [--conditioning-frame <int> ...]  (repeatable; one per pinned frame)
#   [--n-members <N>] [--batch-size <N>]
# ---------------------------------------------------------------------
...
if [ $EXITCODE -ne 0 ]; then
    echo "ERROR: compute_ageostrophic.py failed (exit $EXITCODE)"
    exit $EXITCODE
fi
