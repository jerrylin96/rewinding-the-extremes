...
# ---------------------------------------------------------------------
# BU SCC SGE Directives — Power spectra, full ensemble
#
# Case-agnostic wrapper: parses --variables out of the dispatcher's
# argv and runs compute_power_spectra.py once per variable.
# ---------------------------------------------------------------------
#$ -P eb-general              # Project Name
#$ -N PowerSpec               # Job Name
#$ -cwd                       # Run from the submission directory
#$ -j y                       # Merge Output/Error (file written to power_spectra_logs/, see below)
# h_rt is set via qsub -l flags from dispatch_power_spectra.py
#$ -pe omp 8                  # 8 CPU cores (torch-harmonics SHT + zarr decode)
#$ -l mem_per_core=16G        # 128 GB total — comfortable for batch_size=32 (~1.6 GB/batch)
...
PYTHON_SCRIPT="${SCRIPT_DIR}/compute_power_spectra.py"
...
# 2. Parse arguments
#
# Expected arguments from dispatch_power_spectra.py:
#   --ensemble-zarr <path>
#   --era5-zarr <path>
#   --output-root <path>            (root: <case>/diagnostics/power_spectra)
...
if [ -z "$ENSEMBLE_ZARR" ] || [ -z "$ERA5_ZARR" ] || [ -z "$OUTPUT_ROOT" ] || [ -z "$VARIABLES" ]; then
    ...
    echo "Usage: submit_power_spectra.sh --ensemble-zarr <path> --era5-zarr <path> --output-root <path> --variables <var1,var2,...>"
    exit 1
fi
...
# ---------------------------------------------------------------------
# 3. Loop over variables, calling compute_power_spectra.py once each
# ---------------------------------------------------------------------
...
for idx in "${!VAR_ARRAY[@]}"; do
    ...
    if [ $EXITCODE -ne 0 ]; then
        echo "WARNING: compute_power_spectra.py failed for $VAR (exit code $EXITCODE)"
        ((FAILED+=1))
    fi
done
